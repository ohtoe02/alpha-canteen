...
import CanteenDishCard from '../../ui/cards/canteen-dish-card/CanteenDishCard'
...
function Canteen(): JSX.Element {
	...
	const getRequests = async () => {
		try {
			...
			} = (await get(child(ref(database), `schools/${school}/requests/`))).val()
						...
						{allDishes.map(dish => {
							return (
								<CanteenDishCard
									dish={dish}
									key={dish.id}
									onClick={() => setActiveDish(dish ? dish : null)}
								/>
							)
						})}
					</div>
					...
					<>
						{Object.values(activeDish.classes).map((item: any) => (
							<div key={item.title} className={styles.class}>
								...
								<p>{item.orders} блюда</p>
							</div>
						))}
					</>
				) : (
					<p style={{textAlign:"center"}}>Выберите блюдо в меню слева для отображения заказов этого блюда по классам.</p>
				)}
